			...
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						...
						fieldStyle: $th.attr("fieldStyle") || "",
						cols: $th.attr("cols") || "80",
						rows: $th.attr("rows") || "2"
					};
					fields.push(field);
				});
...
//					if ($btnDel.is("[href^=javascript:]")){
				...
				if (addButTxt) {
					var $addBut = $('<div style="clear: both;" class="button"><div class="buttonContent"><button type="button">'+addButTxt+'</button></div></div>').insertBefore($table).find("button");
					var $rowNum = $('<input type="text" name="dwz_rowNum" class="textInput" style="margin:2px;display:none;" value="1" size="2"/>').insertBefore($table);
					...
					$addBut.click(function(){
						...
						for (var i=0; i<rowNum; i++){
							var $tr = $(trTm);
							var orderNo = $tr.find("td").eq(11).find("input");
							if(orderNo){
								if( "meta[#index#].orderNo" == orderNo.attr("name") ){
									var t = $("#metaDataOrderMax");
									if(t){
										var val = t.attr("value");
										if(val){
											var orderMax = getMaxOrder();
											orderNo.attr("value",parseInt(orderMax) + 1);
											t.attr("value",parseInt(orderMax) + 1);
										}
									}
								}
							}
			...
			function getMaxOrder(){
				...
				if(trcounter > 0){
					max =  $("#trcounter tr").eq(0).find("td").eq(11).find("input").val();
					...
					return $("#metaDataOrderMax").attr("value");
				}
			...
			function tdHtml(field){
				...
				switch(field.type){
					case 'del':
						...
						break;
					case 'btn':
						html = '<input type="button" name="'+field.name+'" value="..." />';
						break;
					case 'lookup':
						var suggestFrag = '';
					...
					case 'date':
						...
						break;
					case 'textarea':
						html = '<textarea name="'+field.name+'" cols="'+field.cols+'" rows="'+field.rows+'" class="'+field.fieldClass+'">'+field.defaultVal+'</textarea>'
							+'<a name="deeTreeSelect" href="javascript:void(0)">选择</a>';
						break;
					default:
						//添加元数据新增，排序号增加
//						var t = $("#metaDataOrderMax");
						var dafault_value = field.defaultVal;
//						if(t){
//							if(field.name == "meta[#index#].orderNo"){
//								var val = $("#metaDataOrderMax").attr("value");
//								if(val){
//									dafault_value = parseInt(val) + 1 ;
//									$("#metaDataOrderMax").attr("value",dafault_value);
//								}
//							}
//						}
						if(field.name == "meta[#index#].orderNo"){
							html = '<input type="text" name="'+field.name+'" onblur=\"CalcOrder(this);\" value="'+dafault_value+'" size="'+field.size+'" class="'+field.fieldClass+'" style="'+field.fieldStyle+'" '+attrFrag+'/>';
						}else{
							html = '<input type="text" name="'+field.name+'" value="'+dafault_value+'" size="'+field.size+'" class="'+field.fieldClass+'" style="'+field.fieldStyle+'" '+attrFrag+'/>';
						}
						break;
				}
				return '<td>'+html+'</td>';
		...
		selectedTodo: function(){
			...
			return this.each(function(){
				...
				$this.click(function(){
					...
					function _doPost(){
                        var _preCallback = $this.attr("preCallback");
                        if (_preCallback) {
                            _preCallback = eval('(' + _preCallback + ')');
                            if (_preCallback() == false) {
                                return false;
                            }
                        }
