...
var alertMsg = {
	...
	_open: function(type, msg, buttons){
		...
		//add by yangyu
		var errorTitle = null;
		if(this._getTitle(type)=="Error")
			errorTitle="错误";
		else
			errorTitle=this._getTitle(type);
		
		var boxHtml = DWZ.frag["alertBoxFrag"].replace("#type#", type).replace("#title#", errorTitle).replace("#message#", msg).replace("#butFragment#", butsHtml);
	...
	_alert: function(type, msg, options) {
		var butTxt = $.regional.alertMsg.butMsg.ok;
		if(butTxt == "OK")
			butTxt = "确定";
		var op = {okName:butTxt, okCall:null};
		$.extend(op, options);
		var buttons = [
