...
function initUI(_box){
	...
	$("form.required-validate", $p).each(function(){
		...
		$form.validate({
			...
			errorPlacement: function(error, element) {
				$("span.error", element.parent()).remove();
				var msg = error.html();
				var errorLabel = "<span generated='true' class='error' title='" + msg + "'></span>";
	...
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			options.closable = eval($this.attr("closable") || "true");
...
//	if ($.fn.sortDrag) $("div.sortDrag", $p).sortDrag(); modify by yangyu
	//if ($.fn.sortDrag) $("div.sortDrag", $p).sortDragAct();
	...
	// 这里放其他第三方jQuery插件...
	//select
	 $("a[selectType=selectOneToShwDiaglog]", $p).each(function(){
			$(this).click(function(event){
				var $this = $(this);
				var title = $this.attr("title") || $this.text();
				var rel = $this.attr("rel") || "_blank";
				var uid = $this.attr("uid") || "_blank";
				
				var ids = "";
				var isFlag = false;
				var $box = navTab.getCurrentPanel();
				$box.find("input:checked").filter("[name='"+uid+"']").each(function(i){
				    var val = $(this).val();
				    if(val == "201211151047"){
				    	//如果是UUID系统函数不能删除
				    	isFlag = true;
				...
				if(ids.indexOf(',')!=-1){
					alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectOneMsg"));
				    return false;  
				}
				var options = {};
				var w = $this.attr("width");
				...
				/********将单选值付给url*******begin****/
				var $tr = $("tr:eq(1)",$box); //从第2个
//				alert($tr.parent().html());
				var sTarget = $tr.attr("target");
				if (sTarget) {
					if ($("#"+sTarget, $tr).size() == 0) {
						$tr.prepend('<input id="'+sTarget+'" type="hidden" />');
					}
					$("#"+sTarget, $tr).val(ids);
				}
				/********将单选值付给url*******end****/
	...
	// navTab
	$("a[selectType=selectOneToShwNavTab]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var title = $this.attr("title") || $this.text();
			var tabid = $this.attr("rel") || "_blank";
	...
	// navTab
	$("a[selectType=selectOneToShwNavTabTwo]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var title = $this.attr("title") || $this.text();
			var tabid = $this.attr("rel") || "_blank";
			...
			tabid = tabid + $box.find("input:checked").filter("[name='"+uid+"']").val();
			title = $box.find("input:hidden").filter("[name='"+$box.find("input:checked").filter("[name='"+uid+"']").val()+"']").val();
			navTab.openTab(tabid, url,{title:title, fresh:fresh, external:external});
