...
(function($){
	$.pdialog = {
		_op:{height:300, width:580, minH:40, minW:50, total:20, max:false, mask:false, resizable:true, drawable:true, maxable:true,minable:true,fresh:true,closable:true},
		_current:null,
		_zIndex:42,
		getCurrent:function(){
		...
		open:function(url, dlgid, title, options) {
			...
			if(dialog) {
				...
				dialog.data("url",url);
                if (op.selObj) {
                    dialog.data("selObj", op.selObj);
                }
                if (op.moreDlg) {
                    dialog.data("moreDlg", op.moreDlg);
                }
...
//					$.pdialog.switchDialog(dialog);
		...
		close:function(dialog) {
			...
			if(editor[dialog.data("id")])
				editor.splice(dialog.data("id"),1);
				
//			for(var i=0;i<editor.length;i++)
//				{
//					if(dialog.data("id")==editor[i].key)
//						editor.splice(i,1);
//				}
