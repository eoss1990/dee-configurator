...
//					}
				...
				$tr.dblclick(function(){                                        
					...
					if($tr.attr("targetType")==""||$tr.attr("targetType")==null)
						return false;
				    if($tr.attr("rel") == "201211151047"){
				    	alertMsg.error("UUID系统函数不能被编辑");
						return false;
				    }
					url=$tr.attr("url")+$tr.attr("rel");
					title=$tr.attr("targetTitle");
					
					if($tr.attr("targetType") == "tab")
						navTab.openTab($tr.attr("targetRel"), url, { title:title, fresh:false, data:{} });
					else if($tr.attr("targetType")=="dialog")
						{
							var options = {
								width: $tr.attr("wth"),
								height: $tr.attr("hth"),
								resizable: $tr.attr("resizable") == "true",
							    maxable:$tr.attr("maxable") == "true"
							};
							$.pdialog.open(url, $tr.attr("targetRel"), title,options);
						}
					else
						alertMsg.error("只能打开标签页或者窗口");
				});
			});
